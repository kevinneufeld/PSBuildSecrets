...
$moduleManifestName = 'PSBuildSecrets.psd1';
$moduleRootPath = "$PSScriptRoot\..\..\Source";
...
Describe 'PSScriptAnalyzer' -Tag 'PSScriptAnalyzer' {
    ...
    Context 'PSbuildSecrets Public and Private Functions' {
        $modulePath = Join-Path -Path $moduleRootPath -ChildPath 'PSBuildSecrets.psm1'
        $moduleManifestPath = Join-Path -Path $moduleRootPath -ChildPath 'PSbuildSecrets.psd1'
        ...
        It 'Should have no Error level PowerShell Script Analyzer violations' {
            ...
            $PSScriptAnalyzerResult += Invoke-ScriptAnalyzer `
                -path (Join-Path -Path $moduleRootPath -ChildPath 'Private\*.ps1') `
